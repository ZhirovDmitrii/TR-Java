...
//	=========================================================
	...
	public boolean removeIf(Predicate<E> pred) {
		...
		int indBuf = 0; // index for save elements in array buf

		for (int i = 0; i < size; i++) {
			if (!pred.test((E) array[i])) // all elements - go test predicate
				buf[indBuf++] = array[i]; // if test() return FALSE - save element
		}
		array = buf; //
		boolean res = indBuf != size; // compare tmp size with old size , and if different - return true
		size = indBuf;
...
//	===== lesson
	...
	// class in class
	private class MyArrayIterator implements Iterator<E> {	// {1,2,3} -> 1 -> 2 -> 3
		int currentIndex = 0; // 1 -> 2 -> 3
		
		@Override
		public boolean hasNext() {
			return currentIndex < size; // 0 < 3 - true - go to next() -> 1 < 3 -> 2 < 3 -> 3 !< 3 FALSE
		}

		@SuppressWarnings("unchecked")
		@Override
		public E next() {
			return (E) array[currentIndex++];	// 0 -> 1 -> 2
		}
